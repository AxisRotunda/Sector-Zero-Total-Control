...
export class StructureRendererService {
  ...
  drawFloorDecoration(ctx: CanvasRenderingContext2D, e: Entity) {
      ...
      // Simple quad projection for flat floor items
      // Coordinates relative to center
      const hw = w/2; 
      const hh = h/2;
      
      const p1 = IsoUtils.toIso(-hw, hh, 0); // Bottom-Left
      const p2 = IsoUtils.toIso(hw, hh, 0);  // Bottom-Right
      const p3 = IsoUtils.toIso(hw, -hh, 0); // Top-Right
      ...
      if (e.subType === 'RUG') {
          ...
          ctx.globalAlpha = 0.5;
          ctx.beginPath();
          ctx.moveTo(-10, 0); ctx.lineTo(5, 5); ctx.lineTo(15, -2); ctx.lineTo(25, 4);
          ctx.stroke();
      }
  ...
  private renderPrism(ctx: any, w: number, d: number, h: number, anchorX: number, anchorY: number, color: string, visuals: ThemeVisuals, detailStyle: string = 'NONE') {
      ...
      // Faces Helper
      const drawFace = (p1: any, p2: any, p3: any, p4: any, shade: number, side: 'LEFT' | 'RIGHT' | 'TOP') => {
          ctx.fillStyle = this.textureGen.adjustColor(color, shade); 
      ...
      // 1. Fill Faces
      drawFace(baseB, baseR, topR, topB, -30, 'RIGHT'); // Right Face (Darkest)
      drawFace(baseB, baseL, topL, topB, -10, 'LEFT');  // Left Face (Mid)
      drawFace(topT, topR, topB, topL, 0, 'TOP');       // Top Face (Lightest)
  ...
  // --- NEW: FACE DETAIL GENERATOR ---
  private drawFaceDetails(ctx: any, pBottom: any, pOuter: any, pTopOuter: any, pTopInner: any, side: 'LEFT' | 'RIGHT', style: string, w: number, h: number) {
      ctx.save();
      // Clip to face
      ctx.beginPath();
      ...
      if (style === 'RIVETS') {
          ...
          for(let i=1; i<steps; i++) {
              ...
              ctx.lineTo(sx + (ex-sx)*0.3, sy + (ey-sy)*0.3 - 20); // Zigzag up
              ctx.lineTo(ex, ey - 20);
              ctx.stroke();
          }
      }
  ...
  private renderHoloTable(ctx: any, e: Entity, w: number, d: number, h: number, ax: number, ay: number) {
      ...
      ctx.globalAlpha = 0.3;
      ctx.beginPath(); ctx.moveTo(-20, 0); ctx.lineTo(0, 30); ctx.lineTo(20, 0); ctx.fill();
